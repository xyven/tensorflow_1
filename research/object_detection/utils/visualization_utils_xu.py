...
def draw_bounding_box_on_image_array(image,
  ...
  """
  #rotate the bounding box add by xu on 2018/11/4
  ...
  draw__inclined_rectangle_on_image(image_pil, newrect, color, thickness)
  np.copyto(image, np.array(image_pil))

def draw__inclined_rectangle_on_image(image,
                               rect,
                               color='red',
                               thickness=4):
  ...
  Args:
    ...
    rect:inclined rectangle of the real(original) bounding box
    color: color to draw bounding box. Default is red.
    thickness: line thickness. Default value is 4.
    use_normalized_coordinates: If True (default), treat coordinates
      ymin, xmin, ymax, xmax as relative to the image.  Otherwise treat
  ...
  rect = np.array(rect) * 460
  #print(rect)
  close_rect = list(rect)
  close_rect.append(rect[0])
  tuple_rect = list(map(tuple, close_rect))
  #print(tuple_rect)
  draw.line(tuple_rect, width=thickness, fill=color)
...
def visualize_boxes_and_labels_on_image_array_xu(
    image,
    boxes,
    classes,
    scores,
